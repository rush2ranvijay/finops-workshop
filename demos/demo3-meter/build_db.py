...
VSCODE_WORKSPACE_STORAGE_DIR = os.path.expanduser(
    os.environ.get("VSCODE_WORKSPACE_STORAGE_DIR", os.path.join(os.environ.get("APPDATA", ""), "Code", "User", "workspaceStorage"))
)
SOURCE_MODE = os.environ.get("FINOPS_SOURCE_MODE", "both").strip().lower()
...
def upsert_session(conn: sqlite3.Connection, session: dict) -> None:
    ...
    conn.execute(
        ...
        INSERT OR REPLACE INTO sessions (
            session_id, source, developer, models, input_tokens, output_tokens, cache_read_tokens,
            cache_write_tokens, total_tokens, nano_aiu, aiu, usd, credits, premium_requests, responses,
            tool_call_count, start_time, end_time, cwd, repository, selected_model, events_path,
            source_logs, has_shutdown
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
...
def load_vscode_chatsessions() -> tuple:
    ...
    source_logs = defaultdict(set)
    parse_errors = []
    pattern = os.path.join(VSCODE_WORKSPACE_STORAGE_DIR, "*", "chatSessions", "*.jsonl")
...
def main() -> int:
    ...
    chat_sessions, chat_parse_errors = load_vscode_chatsessions() if vscode else ({}, [])
    ...
    def merge_sessions(src_sessions: dict, relabel_source: str | None = None) -> None:
        for sid, raw in src_sessions.items():
    ...
    merge_sessions(chat_sessions, relabel_source="chatsessions")
    ...
    meta = {
        ...
        "vscode_chatsessions": len(chat_sessions),
        "merged_sessions": len(merged),
        "raw_log_parse_errors": parse_errors,
        "chatsessions_parse_errors": chat_parse_errors,
