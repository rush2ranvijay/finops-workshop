...
from token_logic import (
    ...
    add_tokens,
    canonical_model,
    tokens_from_token_details_map,
    tokens_from_usage,
    usd_from_tokens,
    zero_tokens,
)
from parsing_logic import (
    extract_json_object_after_key as _extract_json_object_after_key,
    find_logs as _find_logs,
    first_non_empty,
    iso_from_millis,
    parse_copilot_chatsessions_jsonl as _parse_copilot_chatsessions_jsonl,
    parse_copilot_usage_log as _parse_copilot_usage_log,
    parse_iso_timestamp,
    read_events_file,
    safe_get as _safe_get,
    session_for_line_nearest as _session_for_line_nearest,
)


def extract_json_object_after_key(lines: List[str], start_idx: int):
    """Compatibility wrapper for JSON block extraction."""
    return _extract_json_object_after_key(lines, start_idx)


def session_for_line_nearest(anchors, line_no: int):
    """Compatibility wrapper for nearest anchor lookup."""
    return _session_for_line_nearest(anchors, line_no)


def safe_get(dct: Any, *path: str):
    """Compatibility wrapper for safe nested dict access."""
    return _safe_get(dct, *path)


def find_logs(session_id: str, log_dir: str):
    """Compatibility wrapper for log discovery by session id."""
    return _find_logs(session_id, log_dir)


def parse_copilot_usage_log(path: str, target_session: Optional[str] = None):
    """Compatibility wrapper for process-log parser."""
    return _parse_copilot_usage_log(path, target_session)


def parse_copilot_chatsessions_jsonl(path: str, target_session: Optional[str] = None):
    """Compatibility wrapper for chatSessions JSONL parser."""
    return _parse_copilot_chatsessions_jsonl(path, target_session)
